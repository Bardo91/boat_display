...
void ADS1115::writeRegister(uint8_t i2cAddress, uint8_t reg, uint16_t value) {
    write(i2cFd_, (void*)&reg, 1);
    write(i2cFd_, (void*)&value, 2);
}
...
uint16_t ADS1115::readRegister(uint8_t i2cAddress, uint8_t reg) {
    uint16_t result;
    unsigned length = 2;			//<<< Number of bytes to read
    write(i2cFd_, (void*)&reg, 1);
	read(i2cFd_, (void*)&result, length);
...
bool ADS1115::begin()
{
    ...
    if ((i2cFd_ = open(filename, O_RDWR)) < 0) {
        //ERROR HANDLING: you can check errno to see what went wrong
        // printf("Failed to open the i2c bus");
        return false;
    }else{
        if (ioctl(i2cFd_, I2C_SLAVE, ADS1115_DEFAULT_ADDRESS) < 0) {
...
void ADS1115::delay(unsigned _millis){
